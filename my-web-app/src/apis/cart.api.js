...
export const getPayDataAPI = async (cartId) => {
  try {
    ...
    console.error(
      `Error getting payment's data for cartId: ${cartId}:`,
...
export const getPaymentAPI = async (cartId, sessionId) => {
  try {
    ...
    console.error(
      `Error getting payment's for cartId: ${cartId}:`,
      error.response?.data || error.message || error
    );
...
export const removeItemFromCartAPI = async (cartId, itemId) => {
  try {
    ...
    console.error(
      `Error removing item from cart (cartId: ${cartId}, itemId: ${itemId}):`,
      error.response || error.message || error
    );
...
export const updateItemQuantityAPI = async (
  ...
  try {
    const response = await axios.post(
      `${API_BASE_URL}/carts/${cartId}/updateItem`,
      {
        skuId,
        quantity,
        discountPrice,
    ...
    console.error(
      `Error updating item quantity (cartId: ${cartId}, skuId: ${skuId}):`,
      error.response || error.message || error
    );
 ...
 * @returns {Promise<Object>} - API response data.
 */
export const submitCartAPI = async (
  cartId,
  { contactName, email, address, contactPhone, promotionCode }
) => {
  try {
    const response = await axios.post(
      `${API_BASE_URL}/carts/${cartId}/submit`,
      {
        contactName,
        contactEmail: email,
    ...
    console.error(
      `Error submitting cart for cartId: ${cartId}:`,
      error.response || error.message || error
    );
