...
export const getListPromotionCodeAPI = async () => {
  try {
    ...
    console.error(
      `Error getting list promotion for code:`,
      error.response?.data || error.message || error
    );
    throw new Error(
      `Failed to retrieve promotion codes: Error: ${error.message}`
    );
  }
};
...
export const getPromotionCodeAPI = async (code) => {
  ...
  try {
    const response = await axios.get(
      `${API_BASE_URL}/promotion/codes/${code}`,
      {
        headers: getAuthHeaders(),
      }
    );
    ...
    console.error(
      `Error getting promotion for code: ${code}:`,
      error.response?.data || error.message || error
    );
    throw new Error(
      `Failed to retrieve promotion code: ${code}. Error: ${error.message}`
    );
  }
};
